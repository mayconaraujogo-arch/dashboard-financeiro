import { firebaseConfig, CLOUD_DOC_ID } from './firebase-config.js';
...
const KEY='financas_maycon_cloud_v3';
...
let db=null, cloudRef=null, cloudReady=false, applyingCloud=false;

function isFirebaseConfigured(){
 return firebaseConfig && firebaseConfig.apiKey && firebaseConfig.apiKey !== 'COLE_AQUI' && firebaseConfig.projectId !== 'COLE_AQUI';
}

async function setupCloud(){
 if(!isFirebaseConfigured()){
  cloudBanner.textContent='☁️ Modo local — configure o Firebase para sincronizar PC e celular';
  cloudStatus.textContent='Firebase ainda não configurado. Edite firebase-config.js.';
  return;
 }
 try{
  const app=initializeApp(firebaseConfig);
  db=getFirestore(app);
  cloudRef=doc(db,'financeiro',CLOUD_DOC_ID);
  cloudReady=true;
  cloudBanner.textContent='☁️ Sincronização ativa — alterações aparecem no PC e celular';
  cloudStatus.textContent='Nuvem conectada. Sincronização automática ativa.';
  const snap=await getDoc(cloudRef);
  if(!snap.exists()){
    await setDoc(cloudRef,{data:state,updatedAt:new Date().toISOString()});
  }
  ...
  cloudBanner.textContent='⚠️ Erro na nuvem — usando modo local';
  cloudStatus.textContent='Erro ao conectar Firebase: '+e.message;
 }
}

async function syncCloud(){
 if(!cloudReady || !cloudRef || applyingCloud)return;
 state.updatedAt=new Date().toISOString();
 await setDoc(cloudRef,{data:state,updatedAt:state.updatedAt});
}

const save=async()=>{localStorage.setItem(KEY,JSON.stringify(state));render();await syncCloud()};
...
addConta.onclick=()=>{if(!contaNome.value||!contaValor.value)return alert('Preencha nome e valor.');state.contas.push({id:id(),nome:contaNome.value,valor:+contaValor.value,dia:+contaDia.value||5,status:contaStatus.value});contaNome.value=contaValor.value=contaDia.value='';save()};
addGasto.onclick=()=>{if(!gastoDesc.value||!gastoValor.value)return alert('Preencha descrição e valor.');state.gastos.unshift({id:id(),desc:gastoDesc.value,valor:+gastoValor.value,cat:gastoCat.value,data:new Date().toISOString()});gastoDesc.value=gastoValor.value='';save()};
addParcela.onclick=()=>{if(!parcelaNome.value||!parcelaValor.value)return alert('Preencha nome e valor.');state.parcelas.push({id:id(),nome:parcelaNome.value,valor:+parcelaValor.value,restam:parcelaRestam.value?+parcelaRestam.value:null});parcelaNome.value=parcelaValor.value=parcelaRestam.value='';save()};
addMeta.onclick=()=>{if(!metaNome.value||!metaObjetivo.value)return alert('Preencha nome e objetivo.');state.metas.push({id:id(),nome:metaNome.value,atual:+metaAtual.value||0,objetivo:+metaObjetivo.value});metaNome.value=metaAtual.value=metaObjetivo.value='';save()};
saveSalario.onclick=()=>{state.salario=+salarioInput.value||0;save()};
btnSyncNow.onclick=async()=>{await syncCloud();alert('Sincronizado!')};

window.del=(type,itemId)=>{state[type]=state[type].filter(x=>x.id!==itemId);save()};
window.statusConta=(itemId,val)=>{let c=state.contas.find(x=>x.id===itemId);if(c)c.status=val;save()};
window.pagarParcela=(itemId)=>{let p=state.parcelas.find(x=>x.id===itemId);if(p&&p.restam!==null&&p.restam>0)p.restam--;save()};
window.updateGoal=(itemId)=>{let m=state.metas.find(x=>x.id===itemId);let inp=document.getElementById('goal-'+itemId);if(m){m.atual=+inp.value||0;save()}};

exportar.onclick=()=>{const blob=new Blob([JSON.stringify(state,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='backup-financas-maycon.json';a.click()};
importar.onchange=async e=>{const f=e.target.files[0];if(!f)return;try{state=JSON.parse(await f.text());await save();alert('Backup importado!')}catch{alert('Arquivo inválido')}};
...
function renderContas(){
 ...
 const pagas=state.contas.filter(c=>c.status==='Pago').length;contasPagasResumo.textContent=`${pagas}/${state.contas.length} pagas`;
}
...
function renderParcelas(){
 const total=state.parcelas.reduce((a,b)=>a+Number(b.valor||0),0); totalParcelas.textContent=money(total);
...
function goalCard(m){const pct=m.objetivo?Math.min(100,Math.round((m.atual/m.objetivo)*100)):0;return `<div class="goal"><div class="goal-top"><strong>${m.nome}</strong><span>${pct}%</span></div><div class="bar"><div style="width:${pct}%"></div></div><div class="goal-foot"><span>${money(m.atual)}</span><span>${money(m.objetivo)}</span></div><div class="goal-edit"><input id="goal-${m.id}" type="number" step="0.01" placeholder="Novo valor atual"><button class="btn glass" onclick="updateGoal('${m.id}')">Atualizar</button><button class="btn danger" onclick="del('metas','${m.id}')">Excluir</button></div></div>`}
function renderMetas(){listaMetas.innerHTML=state.metas.map(goalCard).join('');dashMetas.innerHTML=state.metas.slice(0,3).map(goalCard).join('')}
function renderAlerts(t){
 let alerts=[]; state.contas.filter(c=>c.status==='Atrasado').forEach(c=>alerts.push(`<div class="alert red"><strong>${c.nome}</strong><br><span>Conta marcada como atrasada.</span></div>`));
...
function render(){
 const t=totals(); sidebarSobra.textContent=money(t.sobra);sidebarStatus.textContent=t.sobra>=0?'Dentro do plano':'Mês negativo';saldoHero.textContent=money(t.sobra);economiaPct.textContent=t.economia+'%';ring.style.setProperty('--pct',t.economia+'%');fraseHero.textContent=t.sobra>=0?'Boa, você ainda tem saldo previsto no mês.':'Atenção: seus gastos passaram do salário.';kpiSalario.textContent=money(state.salario);kpiFixos.textContent=money(t.fixos);kpiExtras.textContent=money(t.gastos);kpiDividas.textContent=money(t.dividas);kpiReserva.textContent=money(t.reserva);salarioInput.value=state.salario;renderContas();renderGastos();renderParcelas();renderMetas();renderAlerts(t);renderCharts(t);
}
render();
setupCloud();
